...
{% endif %}
{% if score == 100 %}
<button type="button" style="float: right;" class="btn success-state animated heartbeat">Score : {{score}} </button>
{% endif %}
{% if score >= 0 and score < 100 %}
<button type="button" style="float: right;" class="btn neutral-state animated heartbeat">Score : {{score}} </button>
{% endif %}
==
