...
Ecrire la fonction `codage(texte, encodage)` ayant en entrée `texte` un texte à coder et `encodage` une 
table d'encodage provenant de l'arbre de huffman et retournant la suite de 0 et de 1 correspondant au texte codé (de type `str`)
