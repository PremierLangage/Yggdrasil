...
textbox =: Input
# NO LONGER NEEDED textbox.type = number
...
evaluator== #|python|
#Q : Que vaut textbox.value is rien n'est mis dans le champ ? None serait bien ! Mais ne serait-ce pas 0 par défaut ?

#Bareme :
#--------
#
#erreur : / 25 si pas d'erreur, /100 si erreur
#type :   / 25 si pas d'erreur
#valeur : / 50 si pas d'erreur
...
def test_reponse():
    ...
    if valeur_expr is None: #Eerreur a l'execution
        ...
        if erreur_RG.selection == "oui":
            ...
            if type_expr == type_RG.selection:
                if type_expr == "int":
                    valeur = int(textbox.value)
                elif type_expr == "float":
                    valeur = float(textbox.value)
                if valeur == valeur_expr:
                    grade = (100, "Bravo")
                else:
                    grade = (50, f"Ok, cette expression ne provoque pas d'erreur à l'execution et est bien du type {type_expr}. Par contre, son évaluation n'est pas {textbox.value} mais {valeur_expr}")
            else:
                grade = (25, f"D'accord, il n'y a pas d'erreur à l'execution, mais le type de l'expression n'est pas {type_RG.value}, mais {type_expr}")
    return grade


grade = correctement_rempli() or test_reponse()
