...
Ecrire une fonction ``occurrences(phrase, mot)`` qui retourne une liste des indices où ``mot`` apparaît tel quel dans ``phrase``. La liste retournée doit être en ordre croissant. 
