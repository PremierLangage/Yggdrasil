...
text==#|markdown|
Écrire une fonction **alloueMaillon** qui prend en argument deux chaînes 
...
Cell* alloueMaillon(char* first_name, char* last_name, int age){
  // Votre code ici...
}
==
...
int main(int argc, char* argv[]){
  ...
  for (i=1 ; i+2<argc ; i+=3){
    c = alloueMaillon(argv[i], argv[i+1], atoi(argv[i+2]));
