...
class OFA(FA):
    ...
    def allRegExps(self):
        ...
        new.trim()
        gfa = new.toGFA()
        for order in itertools.permutations(range(len(gfa.States))):
...
class NFA(OFA):
    ...
    def delTransition(self, sti1, sym, sti2, _no_check=False):
        ...
        if not self.delta[sti1][sym]:
            ...
            if all(map(lambda x: sym not in x, self.delta.values())):
    ...
    def epsilonP(self):
        ...
        :rtype: bool"""
        return any(map(lambda x: Epsilon in x, self.delta.values()))
    ...
    def countTransitions(self):
        ...
        :rtype: int"""
        return sum([sum(map(len, self.delta[t].values()))
                    for t in self.delta])
