...
class SyntaxErrorPL(Exception):
    ...
    def __str__(self):
        return normpath(self.path) + " - " + self.message + " at line " + str(
            self.lineno) + ":\n" + self.line
...
class DirectoryNotFound(Exception):
    ...
    def __str__(self):
        return normpath(self.path) + " -- " + self.message + " : line " + str(
...
class FileNotFound(Exception):
    ...
    def __str__(self):
        return (normpath(self.path) + " " + self.lineno
                + " -- " + self.message
                + " : '" + normpath(self.path_not_found).replace("root", '')
                + "'\n" + self.line)
...
class UnknownExtension(Exception):
    ...
    def __str__(self):
        return normpath(
            self.path) + " -- " + self.message + " : '" + self.ext + "' of '" + self.name + "'"
...
class MissingKey(Exception):
    ...
    def __str__(self):
        return self.message + " : '" + self.key + "' in  " + normpath(self.path)
