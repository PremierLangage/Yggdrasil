...
async def runtests(cmd, feedback, testcases):
    ...
    for (_, testname), result in zip(testcases, results):
        if result == TestStatus.PASS:
            ...
            feedback.addTestError(testname, "Erreur : le programme n'a pas répondu. Causes à considérer : boucle infinie, arrêt prématuré du programme, " +
                                            "oubli d'un retour à la ligne en fin d'écriture sur la sortie standard", 'Validation')
        else:
