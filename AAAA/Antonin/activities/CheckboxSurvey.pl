...
if user__role =="teacher" and number_questions != 0:
    for i in inputValues:           
        ...
        if inputValues[i]["combinations"]: data["Représentation choix : " + inputValues[i]["question"]] = [labels, []]
        data[inputValues[i]["question"]] = [inputValues[i]["items"], []]
        if len(inputValues[i]) > 2:
            if inputValues[i]["combinations"]: data["Représentation choix : " + inputValues[i]["question"]].append(inputValues[i]["horizontal"])
    ...
    for answer in answers:
        ...
        for q, i in json.loads(str(answer.values)).items():
            question, items = i
            if inputValues[i]["combinations"]: data["Représentation choix : " + question][1].append(getValuesAsString(items))
