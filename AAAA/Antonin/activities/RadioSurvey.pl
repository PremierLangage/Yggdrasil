...
if bool(unique_choice) and len(answer) != int(NUMBER_QUESTIONS):
    score = 0
...
if int(score) == 100:
    ...
    feedback = '<span class="error-state">Vous ne pouvez pas sélectionner plusieurs fois la même option{{unique_choice, bool(unique_choice), score}}</span>'
