...
def splitenonce(enonce="nothing"): # 
    ...
    patterntype = re.compile(r'(?P<MI>//\:(?:[\w]+?)\:(?:=(?:.+))//)+?')
    #for line in lines:
...
def defFormMathinput(indice,dico):
    if indice==0:
        ...
        unInput='<p>'+input_prefix+'<span id="math-field" style="font-size:14pt;padding: 0.2em;"></span></p> <inpOut type="text" id="form_math'+str(indice)+'" hidden=true> <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/mathquill/0.10.1/mathquill.min.css">'
    return unInput
