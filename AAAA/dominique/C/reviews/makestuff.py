...
def make_form(namelist): # type: (List[(str,str)]) -> str
    ...
    for name in namelist:
        ...
        if name[0]=='description':
            form += f'<tr><td>{name[0]} <textarea id="form_{name[0]}" colspan="2"></textarea> </td></tr>'
        else:
            form += f'<tr><td {title} >&#9432; {name[0]}  <input {title} class="drs" type=text id="form_{name[0]}" colspan="2"/> </td></tr>'
