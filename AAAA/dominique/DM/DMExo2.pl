...
text==#|markdown|
Écrire une function DEUXIEME **print_terrain** qui prend 
...
si la case est découverte c’est indiqué par un 4 (Action Pied) elle est indiquée par nombre de mines présente dans les 8 cases adjacentes.
...
void print_terrain(int h, int *t[],int l){
    ...
    for(int j=0;j < l ; j++) {
        ...
        switch(t[i][j]){
            ...
            case 4: c= nbmines(h, l, t, i, j); break;
            case 1: c=mine; break;
        }
        printf("%c",c);
        }
