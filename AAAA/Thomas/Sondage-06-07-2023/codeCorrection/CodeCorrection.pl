...
if user__firstname.lower() == "thomas" and user__lastname.lower() == "saillard" :
    ...
    with get_session(table_class = CodeEditorResponse, base=Base) as session:
        ...
        for user, checked in modified_copies:
            ...
            session.query(CodeEditorResponse).join(Response, Response.id == CodeEditorResponse.id).filter(CodeEditorResponse.username == user).update({'checked': int(checked)})
        session.commit()
    ...
    grade = (score, feedback+" Merci "+ (user__username if user__id else user__username + "_" + str(session__id)) + response.__str__())
==
