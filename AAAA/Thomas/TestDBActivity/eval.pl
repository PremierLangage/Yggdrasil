...
with get_session(table_class= Response, base=Base) as session:
    if not session.query(Response).filter(Response.student_id == 0).all() : 
        ...
        session.commit()
    last_user_response = session.query(Response).join(Evaluation, isouter=True).where(Response.student_id != user__id and Evaluation.student_id != user__id).order_by(func.random()).first()
