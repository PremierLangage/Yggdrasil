...
with get_session(table_class= Response, base=Base) as session:
    if not session.query(Response).filter(Response.student_id == 0).all() : 
        ...
        session.commit()
    last_user_response = session.query(Response).where(Response.studnt_id != user__id and ~Response.evaluations.any(Evaluation.student_id == user__id)).order_by(func.random()).first()
