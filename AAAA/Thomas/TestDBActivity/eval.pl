...
with get_session(table_class= Response, base=Base) as session:
    ...
    last_user_response = session.execute(text(f"SELECT responses.id, responses.value, response.user_id FROM test_db_activity_response_3 AS responses LEFT OUTER JOIN test_db_activity_evaluation_3 AS evals ON responses.id = evals.response_id WHERE (evals.student_id != {user__id} OR evals.response_id IS NULL) AND responses.student_id != {user__id};")).first()
